...
const DateInput = ({
  ...
  return (
    <div className={styled(`container ${error ? "isError" : ""}`)}>
      <div className={styled("date-range-input input-group")} ref={ref}>
        <TextInput
          label={label}
          size="xs"
          value={rangeText}
        ...
        />
        <button id={id} className={styled("icon")} onClick={onClickIcon}>
          <CalendarIcon />
        </button>
      </div>
      <div className={styled("errorHint")}>{error}</div>
    </div>
  );
};
