...
function TextInput({
  ...
  return (
    <div
      className={styled(`textInputContainer container ${sizeStyle}${readonly ? " readonly" : ""}`)}
    >
      <input
        ref={ref}
        readOnly={readonly}
        ...
        className={styled(`${value || val ? "haveValue" : ""} ${
          errorMsg ? "haveError" : ""
        }`)}
        onChange={(e) => {
      ...
      {errorMsg ? (
        <span className={styled("errorMessage errorMsg")}>
          {errorMsg}
        </span>
      ) : null}
    </div>
