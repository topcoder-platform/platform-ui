...
class SubmissionsComponent extends React.Component {
  ...
  render() {
    ...
    if (track.toLowerCase() === "design") {
      return challenge.submissionViewable === "true" ? (
        <div className={styled("container view")}>
          <div className={styled("title")}>ROUND 2 (FINAL) SUBMISSIONS</div>
          <div className={styled("content")}>
            {sortedSubmissions.map(renderSubmission)}
          </div>
          {checkpoints.length > 0 && (
            <div className={styled("title")}>ROUND 1 (CHECKPOINT) SUBMISSIONS</div>
          )}
          {checkpoints.length > 0 && (
            <div className={styled("content")}>{checkpoints.map(renderSubmission)}</div>
          )}
        </div>
      ) : (
        <div className={styled("container no-view")}>
          <Lock className={styled("lock")} />
          <div className={styled("title")}>Private Challenge</div>
    ...
    return (
      <div className={styled(`container dev ${isMM ? "" : "non-mm"}`)}>
        {isMM ? (
          <div className={styled("head")}>
            <div className={styled("col-1 col")}>Rank</div>
            <div className={styled("col-2 col")}>User</div>
            <div className={styled("col-3 col")}>Score</div>
        ...
        {!isMM &&
          sortedSubmissions.map((s) => (
            ...
            >
              {!isF2F && !isBugHunt && (
                <div
                  className={styled(`col-2 level-${getRatingLevel(
                    _.get(s.registrant, "rating", 0)
                  )}`)}
                >
              ...
              <div className={styled("col-3")}>
                <a
                  ...
                  rel="noopener noreferrer"
                  className={styled(`handle level-${getRatingLevel(
                    _.get(s.registrant, "rating", 0)
                  )}`)}
                >
                  {_.get(s.registrant, "memberHandle", "")}
