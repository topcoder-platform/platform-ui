...
const ScreeningDetails = ({ screening, helpPageUrl }) => {
  ...
  const warnings = (screening.warnings || []).map((warning, i) => (
    <div className={styled("screening-warning")} key={`${warning.brief}-${i}`}>
      <div className={styled("warning-bold")}>
  ...
  return (
    <div className={styled("screening-details")}>
      <div className={styled("screening-details-head")}>
        <p className={styled(`status-title ${statusInfo.classname}`)}>
          {statusInfo.title}
        </p>
      </div>
      <p>{statusInfo.message}</p>
      <div className={styled("screening-warning")}>
        {warnings}
        {(hasStatusFailed || (hasStatusPassed && hasWarnings)) && (
          <p className={styled("more-info")}>
            Need more info on how to pass screening? Go to help to read Rules &
            Policies.
          </p>
        )}
        <div className={styled("help-btn")}>
          <a
            href={helpPageUrl}
            className={styled("help-link tc-btn-default")}
          >
            Help
          </a>
        </div>
