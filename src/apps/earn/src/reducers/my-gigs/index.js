...
const defaultState = {
  ...
  [GIGS_FILTER_STATUSES.ARCHIVED_JOBS]: {
    myGigs: null,
    page: 1,
    numLoaded: 0,
    total: 0,
...
function onGetMyActiveGigsDone(state, { payload }) {
  const currentGigs =
    payload.page === 1
      ? []
      : state[GIGS_FILTER_STATUSES.ACTIVE_JOBS].myGigs || [];
  return {
    ...state,
    [GIGS_FILTER_STATUSES.ACTIVE_JOBS]: {
...
function onGetMyOpenGigsDone(state, { payload }) {
  const currentGigs =
    payload.page === 1
      ? []
      : state[GIGS_FILTER_STATUSES.OPEN_JOBS].myGigs || [];
  return {
    ...state,
    [GIGS_FILTER_STATUSES.OPEN_JOBS]: {
      myGigs: sortBy(currentGigs.concat(payload.myGigs), ["sortPrio"]),
...
function onGetMyCompletedGigsDone(state, { payload }) {
  const currentGigs =
    payload.page === 1
      ? []
      : state[GIGS_FILTER_STATUSES.COMPLETED_JOBS].myGigs || [];
  return {
    ...state,
    [GIGS_FILTER_STATUSES.COMPLETED_JOBS]: {
      myGigs: sortBy(currentGigs.concat(payload.myGigs), ["sortPrio"]),
...
function onGetMyArchivedGigsDone(state, { payload }) {
  const currentGigs =
    payload.page === 1
      ? []
      : state[GIGS_FILTER_STATUSES.ARCHIVED_JOBS].myGigs || [];
  return {
    ...state,
    [GIGS_FILTER_STATUSES.ARCHIVED_JOBS]: {
      myGigs: sortBy(currentGigs.concat(payload.myGigs), ["sortPrio"]),
