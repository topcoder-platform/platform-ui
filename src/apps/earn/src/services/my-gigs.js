...
const PROFILE_URL = `${config.GIGS_API_BASE_PATH}/profile`;
...
async function getMyGigs(status, page, perPage) {
  const response = await api.get(
    `${config.GIGS_API_BASE_PATH}/myJobApplications?status=${status}&page=${page}&perPage=${perPage}`,
    config.URL.PLATFORM_WEBSITE
  );
...
async function startCheckingGigs(externalId) {
  const res = await api.get(
    `${config.GIGS_API_BASE_PATH}/job?externalId=${externalId}`,
    config.URL.PLATFORM_WEBSITE
  );
  return res;
}
