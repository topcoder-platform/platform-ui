...
import { get, isArray, isObject, map, omit } from "lodash";
...
const ReviewTableLegacy = ({ formData, enableEdit = true }) => {
  ...
  const formatOption = (option) => {
    if (isArray(option)) return option.join(", ");
    if (isObject(option)) {
      return formatOption(get(option, "option", option));
    }
    return option;
  };
  ...
  const renderDetails = (step) => {
    ...
    if (formData?.workType?.selectedWorkType === "Find Me Data") {
      items = omit(items, ["assetsUrl", "goals"]);
    } else {
      items = omit(items, [
        "analysis",
      ...
      {steps
        .filter((s) => {
          if (s.value === "pageDetails")
            return get(formData[s.value], "pages[0].pageDetails") !== "";
