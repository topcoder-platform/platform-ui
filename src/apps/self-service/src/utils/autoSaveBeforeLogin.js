...
import { ACTIONS, AUTO_SAVE_FORM, CACHED_CHALLENGE_ID } from "../config";
...
export const autoSaveCookie = (stateToSave) => {
  ...
  if (progress?.currentStep && isUpdatingTheLatest(form)) {
    ...
    setCookie(AUTO_SAVE_FORM, newCookie, EnvironmentConfig.AUTO_SAVED_COOKIE_EXPIRED_IN);
  }
};
...
export const cacheChallengeId = (challengeId) => {
  if (_.isString(challengeId)) {
    ...
    setCookie(
      CACHED_CHALLENGE_ID,
      encryptedId,
      EnvironmentConfig.AUTO_SAVED_COOKIE_EXPIRED_IN
    );
  }
};
