...
- `Test Challenge` checkbox: shown in Basic Information for both initial setup and later edits. It
  defaults unchecked, reloads from `is_test_challenge`, and explicitly persists metadata value
  `true` or `false`. Test challenges do not generate payments, and authorized modifiers can delete
  them after they reach a completed or cancelled status.
