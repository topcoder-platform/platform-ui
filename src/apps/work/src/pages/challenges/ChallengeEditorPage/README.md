...
- `Test Challenge` checkbox: shown in Basic Information during initial creation, then in Advanced
  Options after the challenge has been created. It defaults unchecked, reloads from
  `is_test_challenge`, and explicitly persists metadata value `true` or `false`. Test challenges do
  not generate payments, and authorized modifiers can delete them after they reach a completed or
  cancelled status.
