...
- `ChallengePrizesField`: placement-prize editor with an inline USD/POINTS selector that uses the challenge editor's green selected state and aligns to the prize-input right edge in both single-prize and multi-prize layouts, descending-value validation for multi-prize setups, create-on-demand placement prize-set hydration when older payloads omit the placement set, and a multi-prize layout that keeps the first row stretched instead of reserving blank space for the delete action.
