...
- `RegisteredMemberDownloadField`: shown in Advanced Options for every created challenge type. The radio group persists `allowAllRegistrantsToDownloadWinningSubmissions` as the exact string `true` for all challenge registrants or `false` for passing submitters only. New challenges explicitly default this metadata to `true`, while existing challenges without the metadata retain passing-submitter-only access.
