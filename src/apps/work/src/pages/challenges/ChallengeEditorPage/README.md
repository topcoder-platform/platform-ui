...
- `ChallengePrizesField`: placement-prize editor with an inline USD/POINTS selector that uses the challenge editor's green selected state and stays right-aligned above the fixed-width prize inputs, descending-value validation for multi-prize setups, create-on-demand placement prize-set hydration when older payloads omit the placement set, and a multi-prize layout that lets the first row absorb the missing delete-action slot so it stays aligned with the selector instead of stopping short.
...
- `Edit` is shown in read-only view mode for existing challenges unless the challenge status is `COMPLETED` or any `CANCELLED*` endpoint status.
