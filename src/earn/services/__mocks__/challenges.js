 ...
 */
export function normalizeChallengeDetails(
  v3,
  v3Filtered,
  v3User,
  v2,
  ...
  if (
    (!challenge.submissions || !challenge.submissions.length) &&
    !_.isEmpty(v2)
  ) {
...
export function normalizeChallenge(challenge, username) {
  const registrationOpen = challenge.allPhases.filter(
    (d) => d.name === "Registration"
  )[0].isOpen
    ? "Yes"
    : "No";
  const groups = {};
...
class ChallengesService {
  ...
  async getChallengeDetails(challengeId) {
    ...
    const challengeV3Filtered = await this.private
  ...
  getChallengeTypes() {
    return Promise.all([
      this.private.apiV2
        .get("/design/challengetypes")
        .then((res) => (res.ok ? res.json() : new Error(res.statusText))),
      this.private.apiV2
        .get("/develop/challengetypes")
        .then((res) => (res.ok ? res.json() : new Error(res.statusText))),
    ]).then(([a, b]) => a.concat(b));
  }
  ...
  getChallenges(filters, params) {
    return this.private
      .getChallenges("/challenges/", filters, params)
      .then((res) => {
  ...
  getUserChallenges(username, filters, params) {
    ...
    return this.private.getChallenges(endpoint, filters, params).then((res) => {
