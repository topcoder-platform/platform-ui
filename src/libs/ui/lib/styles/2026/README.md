...
`:where(.opportunities-app)`, `:where(.gigs-app)`, and `:where(.sales-app)` content containers.
Opportunities, Gigs, and Sales must not apply their theme classes to `body`, so their
