...
const SelfService: FC<{}> = () => {
    ...
    const draftWork = allWork.filter(isDraft)
    const activeWork = allWork.filter(isActive)
    const submittedWork = allWork.filter(isDraft)
    const inReviewWork = allWork.filter(isDraft)
    const redirectedWork = allWork.filter(isDraft)
    const cancelledWork = allWork.filter(isDraft)
    const completeWork = allWork.filter(isDraft)
    const indexedWork: Array<Array<WorkItem>> = [draftWork, activeWork, submittedWork, inReviewWork, redirectedWork, cancelledWork, completeWork]
    let showingStatus = 0
    const workItemStatus = workItem.numOfRegistrants
    const workItemType = workItem.rating
